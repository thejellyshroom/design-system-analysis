...
class MiniRAG:
    ...
    async def ainsert(
        ...
        processed_doc_ids = await self.apipeline_process_enqueue_documents(
            split_by_character, split_by_character_only
        )

        # Entity extraction only for documents processed in this insert (not all PROCESSED)
        inserting_chunks = {}
        if processed_doc_ids:
            processed_map = await self.doc_status.get_docs_by_status(DocStatus.PROCESSED)
            for doc_id in processed_doc_ids:
                status_doc = processed_map.get(doc_id)
    ...
    async def apipeline_process_enqueue_documents(
        ...
        Returns doc ids that reached PROCESSED in this run (for scoped entity extraction).
        """
